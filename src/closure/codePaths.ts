...
export async function computeCodePaths(
  ...
  if (Array.isArray(optionsOrExtraIncludePaths)) {
    void log.warn("'function computeCodePaths(string[])' is deprecated. Use the [computeCodePaths] overload that takes a [CodePathOptions] instead.");
    options = {
...
function allFoldersForPackages(
  ...
  excludedPackages: Set<string>): Promise<Set<string>> {
...
function computeDependenciesDirectlyFromPackageFile(path: string): any {
  ...
  function readFile() {
    try {
      ...
      throw new ResourceError(`Error reading file '${path}' when computing package dependencies. ${err}`);
    }
  }
  ...
  function parse() {
    try {
      ...
      throw new ResourceError(`Error parsing file '${path}' when computing package dependencies. ${err}`);
    }
  }
}
