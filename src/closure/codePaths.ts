...
async function computeCodePathsWorker(
  options: CodePathOptions
): Promise<Map<string, asset.Asset | asset.Archive>> {
...
function allFoldersForPackages(
  ...
  return new Promise((resolve, reject) => {
    readPackageTree(
      ".",
      <any>undefined,
      (err: any, root: readPackageTree.Node) => {
        try {
          if (err) {
            return reject(err);
          }
...
function addPackageAndDependenciesToSet(
  ...
  if (!child) {
    console.warn(
      `Could not include required dependency '${pkg}' in '${upath.resolve(
        root.path
      )}'.`
    );
    return;
...
function findDependency(
  root: readPackageTree.Node | undefined | null,
  name: string
): readPackageTree.Node | undefined {
