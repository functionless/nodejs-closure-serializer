...
use swc_ecma_visit::{Fold, Visit, VisitMut};
use swc_plugin::{ast::*, plugin_transform, utils::*, TransformPluginProgramMetadata};
...
impl ClosureSerializer {
  ...
  fn discover_free_variables() {}
}

impl Visit for ClosureSerializer {
            ...
            ExprOrSpread {
              expr: Box::new(Expr::Arrow(ArrowExpr {
                ...
                body: BlockStmtOrExpr::Expr(Box::new(Expr::Object(ObjectLit {
                  span: DUMMY_SP,
                  props: vec![PropOrSpread::Prop(Box::new(Prop::Shorthand(quote_ident!(
                    "__filename"
                  ))))],
                }))),
                params: vec![],
...
impl LexicalScope {
   ...
   */
  fn bind_var_declarator(&mut self, kind: VarDeclKind, decl: &VarDeclarator) {
