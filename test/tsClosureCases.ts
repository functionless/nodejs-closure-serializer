...
`,
  ...
  cases.push({
      ...
      error:
`Error serializing function 'func'
...
function __assertAsyncThrows(__0) {
  return (function() {
    with({ __awaiter: __f1, assertAsyncThrows: __assertAsyncThrows }) {
...
function compareTextWithWildcards(expected: string, actual: string) {
  ...
  for (
    ...
    if (expectedLine === wildcard) {
      ...
      expect(line.substring(0, index)).toEqual(expectedLine.substring(0, index));
      ...
      expect(line.substring(repetitionIndex)).toEqual(expectedLine.substring(indexAfter));
    } else {
